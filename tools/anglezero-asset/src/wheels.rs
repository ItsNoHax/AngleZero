...
pub fn identify(model: &SourceModel, config: &CarConfig) -> Found {
    ...
    let matched: Vec<usize> = (0..model.parts.len())
        .filter(|&i| {
            let p = &model.parts[i];
            config
                .wheels
...
pub(crate) mod tests {
    ...
    fn corners_are_told_apart_by_where_they_sit() {
        let model = four_wheeled_model();
        let found = identify(&model, &config_matching(&["tyre_", "rim_"]));
    ...
    fn a_pattern_that_finds_three_corners_finds_none() {
        let model = four_wheeled_model();
        let found = identify(&model, &config_matching(&["tyre_fl", "tyre_fr", "tyre_rl"]));
    ...
    fn a_model_with_no_wheel_config_says_so_and_carries_on() {
        let model = four_wheeled_model();
        let found = identify(&model, &CarConfig::unconfigured("Test"));
        assert!(found.wheels.is_empty());
    ...
    fn named_corners_win_over_geometry() {
        ...
        let found = identify(&model, &config);
        assert_eq!(found.wheels.len(), 4);
